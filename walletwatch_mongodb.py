...
document_count = db.expenses.count_documents({})
end_time = time.time()
...
def signup():
    if request.method == "POST":
        ...
        user_data = {"user_email": user_email, "user_password": hashed_password}
    ...
    if request.method == "POST":
        ...
        if user:
            ...
            if bcrypt.checkpw(user_password.encode("utf-8"), stored_password):
                ...
                main_wallet = db.wallets.find_one(
                    {"wallet_user_id": user_id, "wallet_name": "Main"}
                )
                if not main_wallet:
                    wallet_data = {
                        "wallet_name": "Main",
                    ...
                    wallets_users_data = {"wallet_id": wallet_id, "user_id": user_id}
...
def wallets():
    ...
    if request.method == "POST":
        ...
        existing_wallet = db.wallets.find_one(
            {"$or": [{"wallet_name": wallet_name, "wallet_user_id": user_id}]}
        )

        if existing_wallet:
            return (
                jsonify(
                    {"message": "That wallet already exists, try a different name"}
                ),
                409,
            )
        ...
        wallets_users_data = {"wallet_id": new_wallet_id, "wallet_user_id": user_id}
        db.wallets_users.insert_one(wallets_users_data)
...
def close_wallet(wallet_id):
    try:
        ...
        db.wallets.update_one(
            {"_id": ObjectId(wallet_id)}, {"$set": {"wallet_status": "Closed"}}
        )
...
def open_wallet(wallet_id):
    try:
        db.wallets.update_one(
            {"_id": ObjectId(wallet_id)}, {"$set": {"wallet_status": "Open"}}
        )
...
def add_user():
    ...
    if user_email and wallet_id and owner_id is not None:
        ...
        if existing_user:
            existing_wallet_user = db.wallets_users.find_one(
                {"wallet_id": wallet_id, "user_id": existing_user["_id"]}
            )

            if existing_wallet_user:
                return (
                    jsonify({"message": "User already has access to this wallet"}),
                    409,
                )

            db.wallets_users.insert_one(
                {"wallet_id": wallet_id, "user_id": existing_user["_id"]}
            )
...
def remove_user():
    ...
    if user_email and wallet_id and owner_id is not None:
        try:
            ...
            if user:
                wallet_user = db.wallets_users.find_one(
                    {"wallet_id": wallet_id, "user_id": user["_id"]}
                )

                if wallet_user:
                    db.wallets_users.delete_one(
                        {"wallet_id": wallet_id, "user_id": user["_id"]}
                    )
                ...
                return (
                    jsonify({"message": "User is not associated with this wallet"}),
                    404,
                )
            ...
            return (
                jsonify({"message": "An error occurred while deleting the user."}),
                500,
            )
...
def add_money():
    ...
    if not fund_amount or fund_amount == "0.00":
        return (
            jsonify({"success": False, "message": "Please enter a valid amount."}),
            400,
        )
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "message": "An error occurred while adding the amount.",
                }
            ),
            500,
        )
    ...
    if wallet_status != "Open":
        return (
            jsonify(
                {
                    "success": False,
                    "message": "You can only add expenses to open wallets.",
                }
            ),
            403,
        )
...
def get_wallet_id_by_name(expense_wallet):
    try:
        query = {"wallet_name": expense_wallet}
...
def get_balance(user_id):
    try:
        funds_query = {"fund_user_id": user_id}
        ...
        total_funds_amount = (
            total_funds.next()["total_amount"] if total_funds.alive else 0
        )

        expenses_query = {"expense_user_id": user_id}
        ...
        total_expenses_amount = (
            total_expenses.next()["total_amount"] if total_expenses.alive else 0
        )
...
def get_expenses(user_id):
    try:
        expenses_query = [
            ...
            {"$group": {"_id": None, "total_expenses": {"$sum": "$expense_amount"}}},
        ]

        expenses_result = list(db.expenses.aggregate(expenses_query))
        total_expenses = (
            expenses_result[0]["total_expenses"]
            if expenses_result
            else Decimal128("0.00")
        )
...
def get_wallets_list():
    try:
        ...
        query = {
            "$or": [{"wallet_user_id": user_id}, {"shared_users.user_id": user_id}]
        }

        result = db.wallets.find(
            query,
            {"_id": 0, "wallet_name": 1, "wallet_status": 1, "shared_users.user_id": 1},
        )
        ...
        for wallet_info in result:
            ...
            if wallet_status == "Open" or any(
                user.get("user_id") == user_id for user in shared_users
            ):
...
def get_percentages():
    try:
        ...
        funds_query = [
            ...
            {"$group": {"_id": None, "total_funds": {"$sum": "$fund_amount"}}},
        ]
                            ...
                            "$cond": [
                                {
                                    "$eq": [
                                        "$expense_type_id",
                                        ObjectId("64e2a3666fa5cc36f4803a6a"),
                                    ]
                                },
                                "$expense_amount",
                                0,
                            ...
                            "$cond": [
                                {
                                    "$eq": [
                                        "$expense_type_id",
                                        ObjectId("64e2a3666fa5cc36f4803a6b"),
                                    ]
                                },
                                "$expense_amount",
                                0,
                            ...
                            "$cond": [
                                {
                                    "$eq": [
                                        "$expense_type_id",
                                        ObjectId("64e2a3666fa5cc36f4803a6c"),
                                    ]
                                },
                                "$expense_amount",
                                0,
        ...
        expenses_sums = (
            expenses_result[0]
            if expenses_result
            else {"needs_sum": 0, "wants_sum": 0, "savings_debt_sum": 0}
        )
        ...
        needs_percentage = (
            "{:.2f}".format(round((needs_sum / total_funds) * 100, 2))
            if total_funds != 0
            else "0.00"
        )
        wants_percentage = (
            "{:.2f}".format(round((wants_sum / total_funds) * 100, 2))
            if total_funds != 0
            else "0.00"
        )
        savings_debt_percentage = (
            "{:.2f}".format(round((savings_debt_sum / total_funds) * 100, 2))
            if total_funds != 0
            else "0.00"
        )
        ...
        return (
            jsonify({"message": "An error occurred while fetching percentages."}),
            500,
        )
...
def get_expenses_list(wallet_id):
    try:
        expenses = db.expenses.aggregate(
            [
                {"$match": {"expense_wallet_id": ObjectId(wallet_id)}},
                {
                    "$lookup": {
                        "from": "types",
