...
def measure_mysql_read_speed():
    try:
        ...
        query = "SELECT count(*) FROM expenses LIMIT 10"
        ...
        print("count: ", result)
        print("MySQL read speed time: ", read_time)
...
def login():
    if request.method == "POST":
        ...
        if user:
            ...
            if bcrypt.checkpw(
                user_password.encode("utf-8"), stored_password.encode("utf-8")
            ):
                user_id = user[0][0]
                token = jwt.encode(
                    {
                ...
                if not main_wallet:
                    ...
                    query = (
                        "INSERT INTO wallets_users (wallet_id, user_id) VALUES (%s, %s)"
                    )
...
def wallets():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return (
                jsonify({"message": "An error occurred while adding the wallet."}),
                500,
            )
...
def close_wallet(wallet_id):
    try:
        ...
        with connection_pool.get_connection() as connection:
            with connection.cursor() as cursor:
                query = (
                    "UPDATE wallets SET wallet_status = 'Closed' WHERE wallet_id = %s"
                )
    ...
    try:
        ...
        with connection_pool.get_connection() as connection:
            with connection.cursor() as cursor:
                ...
                cursor.execute(
                    query, (user_id, wallet_id, type_id, decimal_amount, name)
                )
                connection.commit()
